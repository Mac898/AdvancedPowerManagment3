...
public class MCreatorLvChargingBenchGUI extends Elementsadvpowermanager3.ModElement {
	public static int GUIID = 2;
	public static HashMap guiinventory = new HashMap();
	public static IInventory inherited;

	public MCreatorLvChargingBenchGUI(Elementsadvpowermanager3 instance) {
		super(instance, 13);
	}
	...
	public static class GuiContainerMod extends Container {
		...
		public GuiContainerMod(World world, int x, int y, int z, EntityPlayer player) {
			...
			if (ent instanceof MCreatorLVChargingBench.TileEntityCustom)
			...
			this.addSlotToContainer(new Slot(inherited, 1, 152, 3) {
				@Override
				public void onSlotChanged() {
					super.onSlotChanged();
			...
			this.addSlotToContainer(new Slot(inherited, 2, 152, 21) {
				@Override
				public void onSlotChanged() {
					super.onSlotChanged();
			...
			this.addSlotToContainer(new Slot(inherited, 3, 152, 39) {
				@Override
				public void onSlotChanged() {
					super.onSlotChanged();
			...
			this.addSlotToContainer(new Slot(inherited, 4, 152, 57) {
				@Override
				public void onSlotChanged() {
					super.onSlotChanged();
			...
			this.addSlotToContainer(new Slot(inherited, 5, 57, 21) {
			});
			this.addSlotToContainer(new Slot(inherited, 6, 75, 21) {
			});
			this.addSlotToContainer(new Slot(inherited, 7, 93, 21) {
			});
			this.addSlotToContainer(new Slot(inherited, 8, 111, 21) {
			});
			this.addSlotToContainer(new Slot(inherited, 9, 57, 39) {
			});
			this.addSlotToContainer(new Slot(inherited, 10, 75, 39) {
			});
			this.addSlotToContainer(new Slot(inherited, 11, 93, 39) {
			});
			this.addSlotToContainer(new Slot(inherited, 12, 111, 39) {
			});
			this.addSlotToContainer(new Slot(inherited, 13, 57, 57) {
			});
			this.addSlotToContainer(new Slot(inherited, 14, 75, 57) {
			});
			this.addSlotToContainer(new Slot(inherited, 15, 93, 57) {
			});
			this.addSlotToContainer(new Slot(inherited, 16, 111, 57) {
			});
			int si;
			int sj;
			for (si = 0; si < 3; ++si)
		...
		private void slotChanged(int slotid, int ctype, int meta) {
			if (this.world != null && this.world.isRemote) {
				advpowermanager3.PACKET_HANDLER.sendToServer(new GUISlotChangedMessage(slotid, x, y, z, ctype, meta));
	...
	public static class GuiWindow extends GuiContainer {
		...
		private static final ResourceLocation texture = new ResourceLocation("advpowermanager3:textures/lvchargingbenchgui.png");
		...
		protected void drawGuiContainerBackgroundLayer(float par1, int par2, int par3) {
			...
			zLevel = 100.0F;
			this.mc.renderEngine.bindTexture(new ResourceLocation("advpowermanager3:textures/lightning bolt.png"));
			this.drawTexturedModalRect(this.guiLeft + 20, this.guiTop + 20, 0, 0, 256, 256);
		}
		...
		protected void actionPerformed(GuiButton button) {
			advpowermanager3.PACKET_HANDLER.sendToServer(new GUIButtonPressedMessage(button.id, x, y, z));
		...
		if (slotID == 0 && changeType == 0) {
			{
				...
				MCreatorChargingBenchRedstoneSlot.executeProcedure($_dependencies);
			}
		}
		if (slotID == 1 && changeType == 0) {
			{
				java.util.HashMap<String, Object> $_dependencies = new java.util.HashMap<>();
				MCreatorChargingBenchUpgradeSlot.executeProcedure($_dependencies);
			}
		}
